...
def find_result_file(model, severity, method):
    candidates = []
...
def collect_mean_results():
    ...
    for model in ["cnn", "resnet18"]:
        for severity in [1, 2, 3, 4, 5]:
            for method in ["frozen", "episodic_tent", "continual_tent"]:
...
def plot_severity_vs_accuracy(mean_df):
    ...
    path = PLOT_DIR / "severity_vs_accuracy.png"
...
def plot_tent_gain(mean_df):
    ...
    for model in ["cnn", "resnet18"]:
        ...
        for method in ["episodic_tent", "continual_tent"]:
            ...
            merged["gain"] = (
                merged["tent_accuracy"]
                - merged["frozen_accuracy"]
            )
    ...
    path = PLOT_DIR / "tent_gain_over_frozen.png"
    plt.savefig(path, dpi=300)
    plt.close()
...
def plot_heatmap(corr_df, model, severity):
    ...
    plt.title(f"{model} Accuracy Heatmap at Severity {severity}")
    ...
    path = PLOT_DIR / f"{model}_heatmap_severity_{severity}.png"
    plt.savefig(path, dpi=300)
    plt.close()
...
def plot_entropy_vs_accuracy(corr_df):
    ...
    path = PLOT_DIR / "entropy_vs_accuracy.png"
    plt.savefig(path, dpi=300)
    plt.close()
...
def print_summary(mean_df):
    ...
    print(
        mean_df
        .sort_values(["model", "method", "severity"])
        [["model", "method", "severity", "mean_accuracy"]]
...
def main():
    ...
    mean_csv_path = PLOT_DIR / "mean_results.csv"
    corr_csv_path = PLOT_DIR / "corruption_results.csv"

    mean_df.to_csv(mean_csv_path, index=False)
    corr_df.to_csv(corr_csv_path, index=False)

    print(f"Saved: {mean_csv_path}")
    ...
    for model in ["cnn", "resnet18"]:
        for severity in [1, 5]:
            plot_heatmap(corr_df, model=model, severity=severity)
